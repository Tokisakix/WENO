...
from models.resnetv1 import resnet_TCGA_KIRC_Encoder, teacher_DSMIL_head, student_head
# from dataset_toy import Dataset_toy
# from Datasets_loader.dataset_MNIST_challenge import MNIST_WholeSlide_challenge
from Datasets_loader.dataset_TCGA_KIRC import TCGA_KIRC_WholeSlide_challenge
import datetime
import utliz
import util
import random
...
class Optimizer:
    ...
    def optimize_teacher(self, epoch):
        ...
        for iter, (data, label, selected) in enumerate(tqdm(loader, desc='Teacher training')):
            ...
            patch_label_pred[selected, :] = instance_attn_score[:, 1].detach().squeeze(0)
...
def get_parser():
    ...
    parser.add_argument('--comment', default='DEBUG_BagDistillationDSMIL', type=str, help='name for tensorboardX')
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter('./runs/runs_TCGA_KIRC/%s'%name)
    writer.add_text('args', " \n".join(['%s %s' % (arg, getattr(args, arg)) for arg in vars(args)]))

    # Setup model
    model_encoder = resnet_TCGA_KIRC_Encoder().to('cuda:0')
    model_teacherHead = teacher_DSMIL_head().to('cuda:0')
    ...
    # Setup loaders
    positive_num = [0, 1, 2, 3]
    negative_num = [4, 5]
    train_ds_return_instance = TCGA_KIRC_WholeSlide_challenge(train=True, positive_num=positive_num, negative_num=negative_num,
    ...
    val_ds_return_instance = TCGA_KIRC_WholeSlide_challenge(train=False, positive_num=positive_num, negative_num=negative_num,
                                        bag_length=args.bag_length, return_bag=False,
                                        num_img_per_slide=args.bag_length, pos_patch_ratio=args.pos_patch_ratio, pos_slide_ratio=0.5, transform=None)
    val_ds_return_bag = TCGA_KIRC_WholeSlide_challenge(train=False, positive_num=positive_num, negative_num=negative_num,
